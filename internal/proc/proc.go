// Package proc runs the external programs herdr delegates to (audio capture,
// speech engines, the agent CLI) and expands the placeholders used in
// user-supplied command templates.
//
// Every externally configurable command in herdr is an argv slice, never a
// shell string. There is no shell interpolation anywhere in herdr, so a device
