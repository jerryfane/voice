...
//
// herdr never links an audio library. Capture and playback are delegated to
...
// host OS exposes is usable without a herdr code change.
package audio

import "context"

// Format is the PCM shape herdr works in end to end: signed 16-bit
...
type Recorder interface {
	...
	Format() Format
	// Describe returns a human-readable device identity for `herdr doctor`.
	Describe() string
}
...
// Device is an enumerated input or output endpoint, used by `herdr devices audio`
