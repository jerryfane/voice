...
func Build(c config.Config) *Assistant {
	...
	return &Assistant{Recorder: rec, Player: player, VAD: seg, STT: stt, TTS: tts, Brain: planner, Devices: device.Build(c), WakePhrases: c.Wake.Phrases, WakeFuzz: c.Wake.Fuzz, FollowUp: c.Wake.FollowUp.D(), Logger: log.New(os.Stderr, "herdr: ", log.LstdFlags)}
}
